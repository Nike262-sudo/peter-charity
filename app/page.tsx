...
function Donation() {
  ...
  const handleDonate = () => {
    ...
    const amount = custom ? `₦${Number(custom).toLocaleString()}` : PRESET_AMOUNTS.find((a) => a.value === selected)?.label || '₦10,000';
    setLoading(true);
    setTimeout(() => {
      setLoading(false);
      ...
      <div className="max-w-2xl mx-auto px-5">
        ...
        <FadeIn delay={0.1}>
          <div className="bg-white rounded-3xl p-8 shadow-sm border border-stone-100">
              ...
              {PRESET_AMOUNTS.map((amt) => (
                <motion.button
                  ...
                  className={`py-3 rounded-xl text-sm font-bold border-2 transition-all ${
                    selected === amt.value && !custom ? 'bg-[#1a4731] border-[#1a4731] text-white' : 'border-stone-200 text-stone-600 hover:border-[#1a4731]/40'
                  }`}
                >
                  {amt.label}
                </motion.button>
            ...
            <p className="text-center text-xs text-stone-400 mt-4">🔒 You will be redirected to complete your donation securely.</p>
          </div>
        </FadeIn>
      </div>
    </section>
